...
doneButton.onclick = async () => {
  ...
  resultContainer.innerHTML = "<p>⚙️ Analyzing your team composition...</p>";
  ...
  try {
    ...
    console.log("Raw API response:", text);
     ...
     // 🧹 Clean up and extract JSON-style result
     let cleanText = text
     .replace(/^{"result":\s*"/, "") // remove start of JSON
     .replace(/"}$/, "") // remove end
     .replace(/\\"/g, '"') // unescape quotes
     .replace(/\\n/g, "\n"); // convert escaped newlines

   // 🎨 Format markdown-like structure into clean HTML
   let formatted = cleanText
     .replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>") // bold
     .replace(/### (.*?)\n/g, "<h3>$1</h3>") // headings
     .replace(/\n\* /g, "<br>• ") // bullet points
     .replace(/\n{2,}/g, "<br><br>") // double newlines -> spacing
     .replace(/\n/g, "<br>"); // single newline

   // ✨ Final styled container
   resultContainer.innerHTML = `
     <div class="result-card fade-in">
       <h2 class="result-title">Recommended Teams</h2>
   ...
   resultContainer.innerHTML = `
     <div class="result-card error">
       <p>❌ Failed to get recommendation. Please try again.</p>
     </div>
   `;
 }
};
